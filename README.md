...
Run these commands from a VS Developer Command Prompt (x64) so MSVC tools are available.
...
If `cmake` or `ctest` are not in PATH, you can use the VS CMake bundle (from a VS Developer Prompt where `$Env:VSINSTALLDIR` is set):
```
